...
class TestTypeTransforms < Minitest::Test
  ...
  def test_complex_argument_types
    ...
    assert_raises(RuntimeError) { complex_argument_types(1.0, [true, "not a bool"]) }
    assert_raises(RuntimeError) { complex_argument_types(true, "string") }
  end
  ...
  def test_named_types
    ...
    assert_raises(RuntimeError) { method_with_named_types([true, 5, "bad"]) }
  end
end
