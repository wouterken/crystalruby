...
class TestTypeDSL < Minitest::Test
  ...
  def test_primitive_union
    ...
    assert_raises(RuntimeError) { iob.value = "not a number or bool" }
  end
  ...
  def test_primitive_array
    ...
    assert_raises(RuntimeError) { ia.value = [1, 2, 3, 4, "not a number or bool or nil"] }
  end
  ...
  def test_primitive_hash
    ...
    assert_raises(RuntimeError) { hash[:not_found] = 33 }
  end
end
