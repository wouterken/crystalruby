...
class TestGCStress < Minitest::Test
  ...
  def test_gc_stress
    ...
    10.times.map do |i|
      puts "Iteration #{i} GC stress v1"
      GCStress.gc_stress_v1(100)
      puts "Iteration #{i} GC stress v2"
      ...
      Thread.new do
        ...
        puts "Thread #{i} gc v2"
        GCStress.gc_stress_v2(100)
        puts "Thread #{i} gc leak v1"
        GCStress.gc_leak_v1(25)
        puts "Thread #{i} gc leak v2"
        GCStress.gc_leak_v2(25)
        puts "Thread #{i} gc trigger v1"
        GCStress.trigger_gc_v1
        puts "Thread #{i} gc trigger v2"
        GCStress.trigger_gc_v2
      end
    end.each(&:join)
    puts "Main thread gc free v1"
    GCStress.gc_free_v1
    puts "Main thread gc free v2"
    GCStress.gc_free_v2
    puts "Main gc trigger v1"
    GCStress.trigger_gc_v1
    puts "Main gc trigger v2"
    GCStress.trigger_gc_v2
  end
end
