module CrystalRuby
  class Library
    ...
    def shards_installed?
      ...
      shards.all? do |k,v|
        dependencies ||= self.shard_file_contents['dependencies']
        dependencies[k] == v
    ...
    def shard_file_contents
      YAML.safe_load(IO.read(shard_file)) rescue { "name" => 'src', "version" => '0.1.0' }
    end
    ...
    def rewrite_shards_file!
      shard_spec = self.shard_file_contents
      dependencies = shard_spec["dependencies"] ||= {}
      updated_shards = dependencies.reject{ |k,v| v["_crystalruby_managed"] }
      ...
      if dirty
        shard_spec["dependencies"] = updated_shards
        shard_spec.delete("dependencies") if updated_shards.empty?
        self.shard_file_contents = shard_spec
      end
    end
    ...
    def attach!
      ...
      singleton_class.class_eval do
        ...
        lib_methods.each_value.select(&:ruby).each do |method|
          attach_function :"register_#{method.name}_callback", %i[pointer], :void
        end
      end
      ...
      Reactor.schedule_work!(self, :init, name, Reactor::ERROR_CALLBACK, Types::Type::ARC_MUTEX.to_ptr, :void, blocking: true, async: false)
